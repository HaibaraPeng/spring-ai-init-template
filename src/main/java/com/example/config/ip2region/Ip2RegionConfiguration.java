package com.example.config.ip2region;


import com.example.config.ip2region.properties.Ip2RegionProperties;
...
public class Ip2RegionConfiguration {
    ...
    public Ip2RegionConfiguration initConfiguration() {
        ...
        try (InputStream classPathFileStream = new ClassPathResource(fileName).getInputStream()) {
            ...
            String tempFile = (StringUtils.endsWith(FileUtils.getTempDirectoryPath(), File.separator) ?
                    FileUtils.getTempDirectoryPath() : FileUtils.getTempDirectoryPath() + File.separator) + fileName;
