...
public enum OpenAiImageType {
    ...
    public static OpenAiImageType getTypeById(Integer id) {
        List<OpenAiImageType> list =
                Arrays.stream(OpenAiImageType.values()).filter(openAiImageType -> Objects.equals(openAiImageType.getId(), id)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    public static OpenAiImageType getTypeByName(String name) {
        List<OpenAiImageType> list =
                Arrays.stream(OpenAiImageType.values()).filter(openAiImageType -> Objects.equals(openAiImageType.name(), name)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
