package com.example.config.ai.spring.service.tts.model.enums;
...
public enum OpenAiTtsType {
    ...
    public static OpenAiTtsType getTypeById(Integer id) {
        List<OpenAiTtsType> list =
                Arrays.stream(OpenAiTtsType.values()).filter(openAiTtsType -> Objects.equals(openAiTtsType.getId(),
                        id)).toList();
        if (list.isEmpty()) {
            return null;
        }
    ...
    public static OpenAiTtsType getTypeByName(String name) {
        List<OpenAiTtsType> list =
                Arrays.stream(OpenAiTtsType.values()).filter(openAiTtsType -> Objects.equals(openAiTtsType.name(),
                        name)).toList();
        if (list.isEmpty()) {
            return null;
        }
