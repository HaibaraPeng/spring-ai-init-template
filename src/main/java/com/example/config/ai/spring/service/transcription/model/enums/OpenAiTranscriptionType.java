...
public enum OpenAiTranscriptionType {
    ...
    public static OpenAiTranscriptionType getTypeById(Integer id) {
        List<OpenAiTranscriptionType> list =
                Arrays.stream(OpenAiTranscriptionType.values()).filter(openAiTranscriptionType -> Objects.equals(openAiTranscriptionType.getId(), id)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    public static OpenAiTranscriptionType getTypeByName(String name) {
        List<OpenAiTranscriptionType> list =
                Arrays.stream(OpenAiTranscriptionType.values()).filter(openAiTranscriptionType -> Objects.equals(openAiTranscriptionType.name(), name)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
