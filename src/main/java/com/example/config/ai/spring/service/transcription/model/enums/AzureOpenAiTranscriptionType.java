package com.example.config.ai.spring.service.transcription.model.enums;
...
public enum AzureOpenAiTranscriptionType {
    ...
    public static AzureOpenAiTranscriptionType getTypeById(Integer id) {
        List<AzureOpenAiTranscriptionType> list =
                Arrays.stream(AzureOpenAiTranscriptionType.values()).filter(azureOpenAiTranscriptionType -> Objects.equals(azureOpenAiTranscriptionType.getId(), id)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    public static AzureOpenAiTranscriptionType getTypeByName(String name) {
        List<AzureOpenAiTranscriptionType> list =
                Arrays.stream(AzureOpenAiTranscriptionType.values()).filter(azureOpenAiTranscriptionType -> Objects.equals(azureOpenAiTranscriptionType.name(), name)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
