...
public enum AzureOpenAiImageType {
    ...
    public static AzureOpenAiImageType getTypeById(Integer id) {
        List<AzureOpenAiImageType> list =
                Arrays.stream(AzureOpenAiImageType.values()).filter(azureOpenAiImageType -> Objects.equals(azureOpenAiImageType.getId(), id)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    public static AzureOpenAiImageType getTypeByName(String name) {
        List<AzureOpenAiImageType> list =
                Arrays.stream(AzureOpenAiImageType.values()).filter(azureOpenAiImageType -> Objects.equals(azureOpenAiImageType.name(), name)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
