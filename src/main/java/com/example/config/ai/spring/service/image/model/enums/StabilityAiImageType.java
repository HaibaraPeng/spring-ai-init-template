package com.example.config.ai.spring.service.image.model.enums;
...
public enum StabilityAiImageType {
    ...
    public static StabilityAiImageType getTypeById(Integer id) {
        List<StabilityAiImageType> list =
                Arrays.stream(StabilityAiImageType.values()).filter(stabilityAiImageType -> Objects.equals(stabilityAiImageType.getId(), id)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    public static StabilityAiImageType getTypeByName(String name) {
        List<StabilityAiImageType> list =
                Arrays.stream(StabilityAiImageType.values()).filter(stabilityAiImageType -> Objects.equals(stabilityAiImageType.name(), name)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
