...
public enum ZhiPuAiImageType {
    ...
    public static ZhiPuAiImageType getTypeById(Integer id) {
        List<ZhiPuAiImageType> list =
                Arrays.stream(ZhiPuAiImageType.values()).filter(zhiPuAiImageType -> Objects.equals(zhiPuAiImageType.getId(), id)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    public static ZhiPuAiImageType getTypeByName(String name) {
        List<ZhiPuAiImageType> list =
                Arrays.stream(ZhiPuAiImageType.values()).filter(zhiPuAiImageType -> Objects.equals(zhiPuAiImageType.name(), name)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
