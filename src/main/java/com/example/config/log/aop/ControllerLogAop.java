package com.example.config.log.aop;
...
import com.example.utils.net.NetUtils;
...
public class ControllerLogAop {
     ...
     */
    private static final String[] MASK_PARAMS = {"password", "checkPassword", "oldPassword", "newPassword",
            "newPassword", "checkNewPassword", "captcha", "token", "email", "passwordCode"};

    /**
     * 记录日志操作用户ID，TransmittableThreadLocal是Alibaba继承ThreadLocal
     * 的一个类，它适用于适用于复杂的线程池、异步任务等场景，由于该日志记录过程中可能会存在这些场景，所以能够保证线程本地变量的传递性。
     */
     ...
     * 定义切入点方法
     */
    @Pointcut("@annotation(com.example.config.log.annotation.ControllerLog)")
    ...
    public void doAfterThrowing(JoinPoint joinPoint, ControllerLog controllerLog, final Throwable e) {
        try {
            ...
            if (e instanceof CustomizeException customizeException) {
                ...
                jsonMap.put("msg",
                        customizeException.getReturnCode().getMsg() + " ==> [" + customizeException.getMsg() + "]");
            } else {
