package com.example.config.easyexcel.convert.date;
...
public class ExcelLocalDateTimeConverter implements Converter<LocalDateTime> {
     ...
     */
    @Override
    public WriteCellData<Object> convertToExcelData(LocalDateTime value, ExcelContentProperty contentProperty,
                                                    GlobalConfiguration globalConfiguration) {
        if (Objects.nonNull(value)) {
            return new WriteCellData<Object>(CellDataTypeEnum.STRING, value.format(DateTimeFormatter.ofPattern("yyyy" +
                    "-MM-dd HH:mm:ss")));
        }
