...
import com.example.config.mail.async.MailAsyncMethod;
import com.example.config.mail.condition.MailCondition;
import com.example.exception.customize.CustomizeMailException;
...
public class MailManager {
     ...
     */
    public void sendWithImageHtmlByClient(String to, String subject, String html, String[] cids,
                                          MultipartFile[] files) {
        // 判断参数是否为空
        ...
        try {
            ...
            for (int i = 0; i < cids.length; i++) {
                String contentType = StringUtils.isNotBlank(files[i].getContentType()) ? files[i].getContentType() :
                        Constants.UNKNOWN_FILE_CONTENT_TYPE;
                String originalName = StringUtils.isNotBlank(files[i].getOriginalFilename()) ?
                        files[i].getOriginalFilename() : files[i].getName();
     ...
     */
    public void sendWithAttachmentByClient(String to, String subject, String text, MultipartFile[] files,
                                           Boolean isHtml) {
        // 判断参数是否为空
