...
public enum ModelTypeService {
    ...
    public static ModelTypeService getEnumById(Integer id) {
        List<ModelTypeService> list =
                Arrays.stream(ModelTypeService.values()).filter(modelTypeService -> Objects.equals(modelTypeService.getId(), id)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    public static ModelTypeService getEnumByName(String name) {
        List<ModelTypeService> list =
                Arrays.stream(ModelTypeService.values()).filter(modelTypeService -> Objects.equals(modelTypeService.getName(), name)).toList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
