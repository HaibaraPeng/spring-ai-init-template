package com.example.utils.redisson.cache;
...
public class CacheUtils {
    ...
    private static <T> void put0(String key, Boolean hasPrefix, T value, Duration expired) {
        ...
        RBucket<T> bucket = REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_PREFIX + key : key);
    ...
    private static List<String> getKeysByPattern0(String keyPattern, Boolean hasPrefix, Integer limit) {
        ...
        KeysScanOptions keysScanOptions = KeysScanOptions
                .defaults()
                .type(RType.OBJECT)
                .pattern(Objects.equals(hasPrefix, Boolean.TRUE) ? KeyPrefixConstants.CACHE_PREFIX + keyPattern :
                        keyPattern);
        ...
        keysByPattern.forEach(Objects.equals(hasPrefix, Boolean.TRUE) ?
                key -> res.add(key.replaceFirst(KeyPrefixConstants.CACHE_PREFIX, "")) : res::add);
        return res;
    }
    ...
    private static Long getExpired0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        long expireTime = REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_PREFIX + key : key).getExpireTime();
        if (expireTime <= 0) {
            return 0L;
        }
    ...
    private static Boolean exists0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_PREFIX + key : key).isExists();
    }
    ...
    private static Boolean delete0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_PREFIX + key : key).delete();
    }
    ...
    private static void deleteByPattern0(String keyPattern, Boolean hasPrefix) {
        verifyParameters(keyPattern);
        REDISSON_CLIENT.getKeys().deleteByPattern(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_PREFIX + keyPattern : keyPattern);
    }
    ...
    private static void putString0(String key, Boolean hasPrefix, CharSequence value, Duration expired) {
        verifyParameters(key, value);
        RBucket<CharSequence> bucket = REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_STRING_PREFIX + key : key, StringCodec.INSTANCE);
        if (Objects.nonNull(expired)) {
            bucket.set(value, expired);
        } else {
            bucket.set(value);
    ...
    private static String getString0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        RBucket<String> bucket = REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_STRING_PREFIX + key : key, StringCodec.INSTANCE);
        return bucket.get();
    }
    ...
    private static List<String> getStringKeysByPattern0(String keyPattern, Boolean hasPrefix, Integer limit) {
        ...
        KeysScanOptions keysScanOptions = KeysScanOptions
                .defaults()
                .type(RType.OBJECT)
                .pattern(Objects.equals(hasPrefix, Boolean.TRUE) ?
                        KeyPrefixConstants.CACHE_STRING_PREFIX + keyPattern : keyPattern);
        if (Objects.nonNull(limit)) {
            keysScanOptions.limit(limit);
        }
        Iterable<String> keysByPattern = keys.getKeys(keysScanOptions);
        ...
        List<String> res = new LinkedList<>();
        keysByPattern.forEach(Objects.equals(hasPrefix, Boolean.TRUE) ?
                key -> res.add(key.replaceFirst(KeyPrefixConstants.CACHE_STRING_PREFIX, "")) : res::add);
        return res;
    }
    ...
    private static Long getStringExpired0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        long expireTime = REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_STRING_PREFIX + key : key, StringCodec.INSTANCE).getExpireTime();
        if (expireTime <= 0) {
            return 0L;
        }
        return (expireTime - System.currentTimeMillis()) / 1000;
    ...
    private static Boolean existsString0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_STRING_PREFIX + key : key, StringCodec.INSTANCE).isExists();
    }
    ...
    private static Boolean deleteString0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_STRING_PREFIX + key : key, StringCodec.INSTANCE).delete();
    }
    ...
    private static void putNumber0(String key, Boolean hasPrefix, Number value, Duration expired) {
        verifyParameters(key, value);
        RBucket<Number> bucket = REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_NUMBER_PREFIX + key : key);
        if (Objects.nonNull(expired)) {
            bucket.set(value, expired);
        } else {
            bucket.set(value);
    ...
    private static List<String> getNumberKeysByPattern0(String keyPattern, Boolean hasPrefix, Integer limit) {
        ...
        KeysScanOptions keysScanOptions = KeysScanOptions
                ...
                .type(RType.OBJECT)
                .pattern(Objects.equals(hasPrefix, Boolean.TRUE) ?
                        KeyPrefixConstants.CACHE_NUMBER_PREFIX + keyPattern : keyPattern);
        if (Objects.nonNull(limit)) {
            keysScanOptions.limit(limit);
        }
        Iterable<String> keysByPattern = keys.getKeys(keysScanOptions);
        ...
        List<String> res = new LinkedList<>();
        keysByPattern.forEach(Objects.equals(hasPrefix, Boolean.TRUE) ?
                key -> res.add(key.replaceFirst(KeyPrefixConstants.CACHE_NUMBER_PREFIX, "")) : res::add);
        return res;
    }
    ...
    private static Long getNumberExpired0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        long expireTime = REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_NUMBER_PREFIX + key : key).getExpireTime();
        if (expireTime <= 0) {
            return 0L;
        }
        return (expireTime - System.currentTimeMillis()) / 1000;
    ...
    private static Boolean existsNumber0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_NUMBER_PREFIX + key : key).isExists();
    }
    ...
    private static Boolean deleteNumber0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getBucket(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_NUMBER_PREFIX + key : key).delete();
    }
    ...
    private static <T> void putList0(String key, Boolean hasPrefix, List<T> valueList, Duration expired) {
        ...
        RList<T> list = REDISSON_CLIENT.getList(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_LIST_PREFIX + key : key);
    ...
    private static <T> List<T> getList0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getList(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_LIST_PREFIX + key : key);
    }
    ...
    private static List<String> getListKeysByPattern0(String keyPattern, Boolean hasPrefix, Integer limit) {
        ...
        KeysScanOptions keysScanOptions = KeysScanOptions
                .defaults()
                .type(RType.LIST)
                .pattern(Objects.equals(hasPrefix, Boolean.TRUE) ? KeyPrefixConstants.CACHE_LIST_PREFIX + keyPattern
                        : keyPattern);
        if (Objects.nonNull(limit)) {
            keysScanOptions.limit(limit);
        }
        ...
        List<String> res = new LinkedList<>();
        keysByPattern.forEach(Objects.equals(hasPrefix, Boolean.TRUE) ?
                key -> res.add(key.replaceFirst(KeyPrefixConstants.CACHE_LIST_PREFIX, "")) : res::add);
        return res;
    }
    ...
    private static Long getListExpired0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        long expireTime = REDISSON_CLIENT.getList(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_LIST_PREFIX + key : key).getExpireTime();
        if (expireTime <= 0) {
            return 0L;
        }
        return (expireTime - System.currentTimeMillis()) / 1000;
    ...
    private static Boolean existsList0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getList(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_LIST_PREFIX + key : key).isExists();
    }
    ...
    private static Boolean deleteList0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getList(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_LIST_PREFIX + key : key).delete();
    }
    ...
    private static void deleteListByPattern0(String keyPattern, Boolean hasPrefix) {
        verifyParameters(keyPattern);
        REDISSON_CLIENT.getKeys().deleteByPattern(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_LIST_PREFIX + keyPattern : keyPattern);
    }
    ...
    private static <T> void putSet0(String key, Boolean hasPrefix, Set<T> valueSet, Duration expired) {
        ...
        RSet<T> set = REDISSON_CLIENT.getSet(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_SET_PREFIX + key : key);
        set.addAll(valueSet);
        if (Objects.nonNull(expired)) {
            set.expire(expired);
        }
    ...
    private static <T> Set<T> getSet0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getSet(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_SET_PREFIX + key : key);
    }
    ...
    private static List<String> getSetKeysByPattern0(String keyPattern, Boolean hasPrefix, Integer limit) {
        ...
        KeysScanOptions keysScanOptions = KeysScanOptions
                .defaults()
                .type(RType.SET)
                .pattern(Objects.equals(hasPrefix, Boolean.TRUE) ? KeyPrefixConstants.CACHE_SET_PREFIX + keyPattern :
                        keyPattern);
        if (Objects.nonNull(limit)) {
            keysScanOptions.limit(limit);
        }
        ...
        List<String> res = new LinkedList<>();
        keysByPattern.forEach(Objects.equals(hasPrefix, Boolean.TRUE) ?
                key -> res.add(key.replaceFirst(KeyPrefixConstants.CACHE_SET_PREFIX, "")) : res::add);
        return res;
    }
    ...
    private static Long getSetExpired0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        long expireTime = REDISSON_CLIENT.getSet(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_SET_PREFIX + key : key).getExpireTime();
        if (expireTime <= 0) {
            return 0L;
        }
        return (expireTime - System.currentTimeMillis()) / 1000;
    ...
    private static Boolean existsSet0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getSet(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_SET_PREFIX + key : key).isExists();
    }
    ...
    private static Boolean deleteSet0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getSet(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_SET_PREFIX + key : key).delete();
    }
    ...
    private static void deleteSetByPattern0(String keyPattern, Boolean hasPrefix) {
        verifyParameters(keyPattern);
        REDISSON_CLIENT.getKeys().deleteByPattern(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_SET_PREFIX + keyPattern : keyPattern);
    }
    ...
    private static <K, V> void putMap0(String key, Boolean hasPrefix, Map<K, V> valueMap, Duration expired) {
        ...
        RMap<K, V> map = REDISSON_CLIENT.getMap(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_MAP_PREFIX + key : key);
        map.putAll(valueMap);
        if (Objects.nonNull(expired)) {
            map.expire(expired);
        }
    ...
    private static List<String> getMapKeysByPattern0(String keyPattern, Boolean hasPrefix, Integer limit) {
        ...
        KeysScanOptions keysScanOptions = KeysScanOptions
                .defaults()
                .type(RType.MAP)
                .pattern(Objects.equals(hasPrefix, Boolean.TRUE) ? KeyPrefixConstants.CACHE_MAP_PREFIX + keyPattern :
                        keyPattern);
        if (Objects.nonNull(limit)) {
            keysScanOptions.limit(limit);
        }
        ...
        List<String> res = new LinkedList<>();
        keysByPattern.forEach(Objects.equals(hasPrefix, Boolean.TRUE) ?
                key -> res.add(key.replaceFirst(KeyPrefixConstants.CACHE_MAP_PREFIX, "")) : res::add);
        return res;
    }
    ...
    private static Long getMapExpired0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        long expireTime = REDISSON_CLIENT.getMap(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_MAP_PREFIX + key : key).getExpireTime();
        if (expireTime <= 0) {
            return 0L;
        }
        return (expireTime - System.currentTimeMillis()) / 1000;
    ...
    public static Boolean existsMap0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getMap(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_MAP_PREFIX + key : key).isExists();
    }
    ...
    private static Boolean deleteMap0(String key, Boolean hasPrefix) {
        verifyParameters(key);
        return REDISSON_CLIENT.getMap(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_MAP_PREFIX + key : key).delete();
    }
    ...
    private static void deleteMapByPattern0(String keyPattern, Boolean hasPrefix) {
        verifyParameters(keyPattern);
        REDISSON_CLIENT.getKeys().deleteByPattern(Objects.equals(hasPrefix, Boolean.TRUE) ?
                KeyPrefixConstants.CACHE_MAP_PREFIX + keyPattern : keyPattern);
    }
