...
import com.example.exception.customize.CustomizeEncryptException;
...
public class SHA3Utils {
    ...
    public static String encrypt(byte[] plainByteArray, byte[] saltByteArray) {
        try {
            ...
            byte[] digest = md.digest(ArrayUtils.isEmpty(saltByteArray) ? plainByteArray :
                    ArrayUtils.addAll(plainByteArray, saltByteArray));
