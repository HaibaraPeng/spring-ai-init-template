...
import com.example.model.vo.user.UserExportVo;
import com.example.model.vo.user.UserPageVo;
import com.example.service.UserService;
import com.example.utils.document.excel.ExcelUtils;
import com.example.utils.oss.minio.MinioUtils;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public Page<UserPageVo> pageUser(UserPageDto userPageDto, PageModel pageModel) {
        ...
        List<UserPageVo> newRecords = page.getRecords().stream().map(user -> {
            File avatarFile = Objects.equals(user.getAvatarId(), Constants.NULL_ID) ? null :
                    fileMapper.selectById(user.getAvatarId());
    ...
    public void importUser(MultipartFile file) {
        try {
            List<UserImportDto> userImportDtoList = ExcelUtils.importStreamSyncAndClose(file.getInputStream(),
                    UserImportDto.class);
            List<User> list = userImportDtoList.stream().map(user -> {
                if (StringUtils.isBlank(user.getAccount())) {
                    throw new CustomizeReturnException(ReturnCode.EXCEL_FILE_ERROR, "存在账号[账号：" + user.getAccount() +
                            " 邮箱：" + user.getEmail() + " 名称：" + user.getName() + "]不完整");
                }
                if (!user.getAccount().matches("^[A-Za-z0-9]{2,16}$")) {
                    throw new CustomizeReturnException(ReturnCode.EXCEL_FILE_ERROR, "存在账号[" + user.getAccount() +
                            "]格式错误，要求介于2-16位且不包含特殊字符");
                }
                if (StringUtils.isBlank(user.getEmail())) {
                    throw new CustomizeReturnException(ReturnCode.EXCEL_FILE_ERROR, "存在邮箱[账号：" + user.getAccount() +
                            " 邮箱：" + user.getEmail() + " 名称：" + user.getName() + "]不完整");
                }
                if (!Constants.REGEX_EMAIL_PATTERN.matcher(user.getEmail()).matches()) {
                    throw new CustomizeReturnException(ReturnCode.EXCEL_FILE_ERROR, "存在邮箱[" + user.getEmail() +
                            "]格式错误");
                }
                if (StringUtils.isBlank(user.getName())) {
                    throw new CustomizeReturnException(ReturnCode.EXCEL_FILE_ERROR, "存在名称[账号：" + user.getAccount() +
                            " 邮箱：" + user.getEmail() + " 名称：" + user.getName() + "]不完整");
                }
                if (!user.getName().matches("^.{1,16}$")) {
                    throw new CustomizeReturnException(ReturnCode.EXCEL_FILE_ERROR, "存在名称[" + user.getName() +
                            "]格式错误，要求介于1-16位");
                }
                LambdaQueryWrapper<User> userLambdaQueryWrapper = new LambdaQueryWrapper<>();
                userLambdaQueryWrapper
                ...
                if (userMapper.exists(userLambdaQueryWrapper)) {
                    throw new CustomizeReturnException(ReturnCode.USERNAME_ALREADY_EXISTS,
                            "账号或工号已经存在[账号：" + user.getAccount() + " 名称：" + user.getName() + "]");
                }
                return new User()
                        .setAvatarId(Constants.NULL_ID)
                        .setAccount(user.getAccount())
            ...
            if (!Objects.equals(userImportDtoList.size(), list.size())) {
                throw new CustomizeReturnException(ReturnCode.USERNAME_ALREADY_EXISTS,
                        "存在" + (userImportDtoList.size() - list.size()) + "条重复数据");
            }
