...
public class LogServiceImpl extends ServiceImpl<LogMapper, Log> implements LogService {
    ...
    public Page<LogPageVo> pageLog(LogPageDto logPageDto, PageModel pageModel) {
        ...
        logLambdaQueryWrapper
                ...
                .eq(StringUtils.isNotBlank(logPageDto.getRequestMethod()), Log::getRequestMethod,
                        logPageDto.getRequestMethod())
                ...
                .like(StringUtils.isNotBlank(logPageDto.getDescription()), Log::getDescription,
                        logPageDto.getDescription())
        ...
        List<LogPageVo> newRecords = page.getRecords().stream().map(log -> new LogPageVo()
                ...
                .setUserAccount(Objects.equals(log.getUserId(), Constants.NULL_ID) ? "该操作不记录用户信息" :
                        (Objects.nonNull(userMapper.selectById(log.getUserId())) ?
                                userMapper.selectById(log.getUserId()).getAccount() : "用户信息不存在"))
                .setIp(log.getIp())
    ...
    public List<LogExportVo> exportExcelList() {
        ...
        return logsInDatabase.stream().map(log -> {
            ...
            logExportVo.setUserAccount(Objects.equals(log.getUserId(), Constants.NULL_ID) ? "该操作不记录用户信息" :
                    (Objects.nonNull(userMapper.selectById(log.getUserId())) ?
                            userMapper.selectById(log.getUserId()).getAccount() : "用户信息不存在"));
